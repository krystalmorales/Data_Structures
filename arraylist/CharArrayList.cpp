...
void CharArrayList::concatenate(CharArrayList *other) {
    int newCapacityRequired = numItems + other->numItems;

        if (newCapacityRequired > capacity) {
        int newCapacity = std::max(2 * capacity, newCapacityRequired);
