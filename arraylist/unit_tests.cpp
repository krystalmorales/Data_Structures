...
// Test concatenation of empty CharArrayList
void test_concatenate_empty_with_nonEmpty() {
    CharArrayList emptyList;

    char arr_non_empty[5] = { 'H', 'e', 'l', 'l', 'o' };
    CharArrayList nonEmptyList(arr_non_empty, 5);

    emptyList.concatenate(&nonEmptyList);

    assert(emptyList.size() == 5); 
    assert(emptyList.toString() == "[CharArrayList of size 5 <<Hello>>]");

}

// Test for concatenates a non empty char array list with itself  
void test_concatenation_nonEmpty_with_itself() {
    char arr_non_empty[5] = { 'H', 'e', 'l', 'l', 'o' };
    CharArrayList nonEmptyList(arr_non_empty, 5);

    CharArrayList originalList = nonEmptyList;

    originalList.concatenate(&nonEmptyList);

    assert(originalList.size() == 10);
    assert(originalList.toString() == "[CharArrayList of size 10 <<HelloHello>>]"); 
}
...
void test_assignment_operator() {
    ...
    assert(test_list1.toString() == "[CharArrayList of size 5 <<World>>]");
    assert(test_list2.toString() == "[CharArrayList of size 5 <<World>>]");
